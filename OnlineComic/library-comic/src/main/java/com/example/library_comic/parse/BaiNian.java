...
public class BaiNian extends ComicSource {

    private static final String searchUrl = "https://m.bnman.net/";

    @Override
    public int getSourceId() {
        return SourceEnum.BAINIAN.ID;
    }

    @Override
    public String getSourceName() {
        return SourceEnum.BAINIAN.NAME;
    }

    @Override
    public String getSourceHost() {
        return "https://m.bnman.net";
    }
    ...
    public List<Comic> parseRecentUpdateList(String html) {
        try {
            List<Comic> list = new ArrayList<>();
            List<Node> nodes = new Node(html).list("[class=tbox]");
            Elements elements = null;
            for (Node node : nodes) {
                Logger.i(node.get().toString() + "=========");
                String label = node.getChild("[class=tbox_t]").text("h3").trim();
                if (label.contains("最近更新")) {
                    elements = node.getChild("[class=tbox_m]").get().select("li > a");
                    break;
                }
            }
            for (Element e : elements) {
                ...
                String coverUrl = node.attr("mip-img", "src");
                Comic comic = new Comic(title, getSourceHost() + href, coverUrl);
                list.add(comic);
                Logger.i(href + "----" + title + "----" + coverUrl);
            }
            return list;
        } catch (Exception e) {
            e.printStackTrace();
